<nav class="app-header navbar navbar-expand bg-body position-sticky top-0">
    <div class="container-fluid">
        ...
        <ul class="navbar-nav">
            ...
            @foreach (auth()->user()->role == 'admin' ? config('menu.admin.navbar') : config('menu.manager.navbar') as $title => $link)
                @php
                    $active = request()->routeIs($link);
                    $linkHref = Route::has($link) ? route($link) : $link;
                @endphp
                <li class="nav-item d-none d-md-block">
                    <a href="{{ $linkHref }}" class="nav-link {{ $active ? 'active' : '' }}">{{ $title }}</a>
                </li>
            @endforeach
        ...
        <ul class="navbar-nav ms-auto">
            ...
            {{-- <li class="nav-item">
                <a class="nav-link" data-lte-toggle="navbar-search" href="#" role="button">
